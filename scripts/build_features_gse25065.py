"""Build TME features for GSE25065 — analog build_features.py для GEO.

Загрузка → ssGSEA → TME-панель → validation → parquet.
"""
...
from oncotme.cohorts.gse25065 import load_gse25065
...
logger = logging.getLogger("build_features_gse25065")


def _hard_gate(report: ValidationReport, step: str) -> None:
    counts = report.counts()
    logger.info("[%s] PASS=%d WARN=%d FAIL=%d", step,
...
def _parse() -> argparse.Namespace:
    ...
    p.add_argument(
        "--series-matrix",
        default="/Users/dianalysenko/Documents/DrugResponce/data/raw/geo/GSE25065_series_matrix.txt.gz",
    )
    p.add_argument(
        "--gpl96",
...
def main() -> int:
    ...
    logger.info("[1/3] Loading GSE25065 …")
    cohort = load_gse25065(args.series_matrix, args.gpl96)
    ...
    logger.info("Features: %s (sig=%d, gene=%d, clin=%d)",
                features.shape,
                sum(1 for c in features.columns if c.startswith("sig__")),
                sum(1 for c in features.columns if c.startswith("gene__")),
                sum(1 for c in features.columns if c.startswith("clin__")))
    ...
    logger.info("[3/3] Done. Artefacts → %s", out_dir)
